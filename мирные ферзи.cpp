...
//x, y - координаты клетки, на которую мы хотим поставить нового ферзя  
{
    ...
    //проверки:  2 ферзя на одной строке, диагонали            или если нет ферзя в x1-ом столбце    
    for (x1 = 1; x1 < x && yi[x1] != y && (abs(x1 - x) != abs(yi[x1] - y) || yi[x1] == 0); x1++);
